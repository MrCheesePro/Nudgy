...
import { flushSamples, getAppTotals, getUsageBreakdown, listUnmappedProcesses } from "../lib/ipc";
import { dayBounds } from "../lib/time";
import type { AppTotal, UnmappedProcess, UsageBreakdown } from "../lib/types";
...
export function useUsageStats() {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const [nextBreakdown, nextApps, nextUnmapped] = await Promise.all([
  ...
  return { breakdown, apps, unmapped, error, refresh };
}
